...
``cfonts_to_python.py`` Converts a "C" code file generated by GLC Font Creator into a Python file
...
With frozen fonts the fonts are store in Flash as part of the device firmware. This enables them to
be accessed as a ``bytes`` instance with faster operation. It uses even less RAM than file access
at the cost of having to compile the firmware from source.
...
solution is to incorporate the module as part of the firmware: fonts can be employed with minimal
RAM uses.
...
battery power. To achieve this, external hardware can be used to turn off all power to the display
and other peripherals before putting the Pyboard into standby. On waking from standby the program
turns on power to the peripherals, turning the power off before going back into standby. For the
lowest possible consumption an SD card should not be installed on the Pyboard as this consumes
power at all times. Fonts and images should be stored in the Pyboard flash memory or on an external
device whose power is switched (such as the flash memory on a power-switched display).
...
code into firmware. Modules can be frozen as .py files or compiled to bytecode and frozen in that
form. The former method is currently required for code which employs Viper and Native decorators
(such as epd.py and epaper.py).
