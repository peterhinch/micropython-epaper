...
# END OF USER INTERFACE
    ...
    def __enter__(self):                        # power up sequence
        ...
        # read the COG ID 
        cog_id = self._SPI_read((0x71, 0x00)) & 0x0f
        ...
        # Disable OE
        self._SPI_send((0x70, 0x02))
        self._SPI_send((0x72, 0x40))

        # check breakage
        self._SPI_send((0x70, 0x0f))
        broken_panel = self._SPI_read((0x73, 0x00)) & 0x80
        if broken_panel == 0:
            self.status = EPD_PANEL_BROKEN
            self._power_off()
            raise EPDException("EPD COG device reports broken status")
        # power saving mode
        self._SPI_send((0x70, 0x0b))
        self._SPI_send((0x72, 0x02))
        # channel select
        self._SPI_send((0x70, 0x01))
        self._SPI_send((0x72, 0x00, 0x00, 0x00, 0x7f, 0xff, 0xfe, 0x00, 0x00)) # Channel select
        # high power mode osc
        self._SPI_send((0x70, 0x07))
        self._SPI_send((0x72, 0xd1))
        # power setting
        self._SPI_send((0x70, 0x08))
        self._SPI_send((0x72, 0x02))
        # Vcom level
        self._SPI_send((0x70, 0x09))
        self._SPI_send((0x72, 0xc2))
        # power setting
        self._SPI_send((0x70, 0x04))
        self._SPI_send((0x72, 0x03))
        # driver latch on
        self._SPI_send((0x70, 0x03))
        self._SPI_send((0x72, 0x01))
        # driver latch off
        self._SPI_send((0x70, 0x03))
        self._SPI_send((0x72, 0x00))

        self.delay(5)
        dc_ok = False
        for i in range(4):
            # charge pump positive voltage on - VGH/VDL on
            self._SPI_send((0x70, 0x05))
            self._SPI_send((0x72, 0x01))
            self.delay(240)
            ...
            self._SPI_send((0x70, 0x05))
            self._SPI_send((0x72, 0x0f))
            self.delay(40)
            # check DC/DC
            self._SPI_send((0x70, 0x0f))
            dc_state = self._SPI_read((0x73, 0x00)) & 0x40
            if dc_state == 0x40:
                dc_ok = True
                # output enable to disable
                self._SPI_send((0x70, 0x02))
                self._SPI_send((0x72, 0x04))
                break
        if not dc_ok:
            # output enable to disable
            self._SPI_send((0x70, 0x02))
...
# The 2.7” frame of data is 176 lines * 264 dots.
    ...
    def _frame_stage2(self):
        for i in range(self.compensation['stage2_repeat']):
            self._frame_fixed_timed(0xff, self.compensation['stage2_t1'])
            self._frame_fixed_timed(0xaa, self.compensation['stage2_t2'])
...
# Optimisation: display refresh code spends 98.5% of its time running _line() [97.8% after optimisation]
    ...
    def _line_fixed(self, line, fixed_value, set_voltage_limit):
        ...
        if set_voltage_limit:               # charge pump voltage level reduce voltage shift
            self._SPI_send((0x70, 0x04, 0x72, 0)) # voltage level 0 for 2.7 inch panel
        self._SPI_send((0x70, 0x0a))
        ...
        self._SPI_send((0x70, 0x02, 0x72, 0x07))
    ...
    def _setbuf_fixed(self, line: int, fixed_value: int):
        index = 0                           # odd pixels
        buf = self.linebuf
        image = self.image # Bizarrely removing this redundant line made the update 0.5S slower (?????) yet timing a function call yielded 275uS with or without the line
    ...
    def _setbuf_data(self, line: int, offset: int): # 5.85S
        ...
        buf = self.linebuf                # Optimisation: use local namespace
        image = self.image
        index = 0                           # odd pixels
